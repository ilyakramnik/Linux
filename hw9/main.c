...
int main() {
    ...
    if (process < 0) {
        ...
        for (int i = 1; i <= 10; ++i) {
            ...
            if (sem_post(sem) == -1) { // проверяем успешность вызова sem_post
                perror("Error: Cannot post semaphore in parent.\n");
                exit(1);
            }

            if (sem_wait(sem) == -1) { // проверяем успешность вызова sem_wait
        ...
        // закрытие канала и семафора в parent
        if (close(fd[0]) == -1) {
        ...
        for (int i = 1; i <= 10; ++i) {
            ...
            if (sem_post(sem) == -1) { //
                // проверяем успешность вызова sem_post
                perror("Error: Cannot post semaphore in child.\n");
                exit(1);
            }
        }

        // закрытие канала и семафора в child
        if (close(fd[0]) == -1) {
